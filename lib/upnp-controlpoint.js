...
var TRACE = false;
var DETAIL = false;
var DEVICE_SEARCH_TIMEOUT = 5; // 5 seconds
...
var UpnpControlPoint = function() {
	...
	this.current_devices = [];  // current devices
	this.timeout = setTimeout();  // current device timeout
	...
	this.ssdp.on("DeviceFound", function(device) {
		...
		self.current_devices.push(udn); // add to current device list
		clearTimeout(self.timeout); // reset timeout
		self.timeout = setTimeout(resetDevices, 1000 * DEVICE_SEARCH_TIMEOUT, self);
...
UpnpControlPoint.prototype.search = function(s) {
	...
	clearTimeout(this.timeout); // reset current device timeout
	...
	var req = http.request(options, function(res) {
		...
		res.on('end', function () {
			...
			xml2js.parseString(resData, function(err, result) {
                if (err || typeof result === 'undefined' || typeof result.root === 'undefined' || !result.root) {
